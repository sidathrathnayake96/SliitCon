...
class approveConference extends Component {
  ...
  navigateApproveConference(e, confName) {
    e.preventDefault();
    axios.put(`http://localhost:5000/conference/updateAcStatus/${this.props.match.params.confName}`)
    .then(response => { 
      if(response.status===200){
        window.location='/adminViewConf'
        alert('Conference is Approved!!!')
      }
      else{
        alert('Error!!!')
      }
    ...
    return (
      <div>
        ...
        <div className="container">
          <form className="form">
            <h2 id="ConfH">APPROVE CONFERENCE</h2><br/>
            ...
            <div className="action-wrapper text-center">
              <div className="action-btn form-group">
                <button className="form-btn btn mb-2" id="confBtns" type="submit" onClick={e => this.navigateApproveConference(e, this.state.conference.conferenceName)}>Confirm Approve</button>
              </div>
            </div>
          </form>
        </div>
