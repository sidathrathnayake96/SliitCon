...
export default class ReviewerResetpassword extends Component {
...
reviewerResetpasswordHandler = async (e) => {
    e.preventDefault();

    const {reviewerPassword,reviewerConfirmPassword }= this.state;
    ...
    if( this.state.reviewerPassword !== this.state.reviewerConfirmPassword){
        setTimeout((err)=>{
    ...
    await axios.put(`http://localhost:5000/reviewer/reviewerresetpassword/${this.props.match.params.resetToken}`,data).then((res) =>{
...
<div className="form-group">
                                    ...
                                    <div className="form-group">
                                        ...
                                        <input 
                                            type="password" 
                                            id="reviewerConfirmPassword" 
                                            value={this.state.reviewerConfirmPassword} 
                                            onChange={this.handleInputChange}
                                            name="reviewerConfirmPassword"  
                                            placeholder="Confirm Password..."
                                            required/>
                                    </div><br/><hr/>
                           
                            <div className="form-group" style={{marginLeft:'30%'}}>
                                <button type="submit" className="form-btn-resetpassword" onClick={this.reviewerResetpasswordHandler}>Reset Password</button>
                            </div>         
