...
export default class ResearcherDataEdit extends Component{
...
componentDidMount(){
    ...
    axios.get(`http://localhost:5000/researcher/researcherdata/${localStorage.getItem('researcherId')}`).then((res) =>{
        if(res.data.success){
            this.setState({
...
onSubmit = (e) => {
    ...
    this.props.history.push(`/researcherdata/${localStorage.getItem("researcherEmail")}`)
}
