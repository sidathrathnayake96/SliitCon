...
export default class ReviewerForgotpassword extends Component{
    ...
    reviewerForgotpasswordHandler = async (e) => {
        e.preventDefault();
 
        const {reviewerEmail} = this.state;

        const data = {
            reviewerEmail: reviewerEmail
        } 
       
            await axios.post("http://localhost:5000/reviewer/reviewerforgotpassword", data).then((res) =>{
...
<button type="submit" className="form-btn-forgotpassword" onClick={this.reviewerForgotpasswordHandler}>Send Email</button>
</div>  
