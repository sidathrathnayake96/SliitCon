...
export default class ResearcherForgotpassword extends Component{
    ...
    researcherForgotpasswordHandler = async (e) => {
        e.preventDefault();
 
        const {researcherEmail} = this.state;

        const data = {
            researcherEmail: researcherEmail
        } 
       
            await axios.post("http://localhost:5000/researcher/researcherforgotpassword", data).then((res) =>{
...
<button type="submit" className="form-btn-forgotpassword" onClick={this.researcherForgotpasswordHandler}>Send Email</button>
</div>  
