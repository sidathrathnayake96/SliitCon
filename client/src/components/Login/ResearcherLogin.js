...
export default class ResearcherLogin extends Component {
constructor (props){
    super(props);
...
researcherLoginHandler = (e) => {
    e.preventDefault();

    
    const { researcherEmail, researcherPassword } = this.state;
    ...
    const data = {
        ...
        axios.post("http://localhost:5000/researcher/researcherlogin",data).then((res) =>{
            if(res.data.success){
                localStorage.setItem("researcherToken", res.data.token);
                localStorage.setItem("researcherEmail", data.researcherEmail);
                this.props.history.push(`/researcherhome`);
<<<<<<< HEAD
=======
            }else{
            ...
            <h1 htmlFor="role" className="text-dark" style={{textAlign:"center", fontWeight:"bolder", fontFamily:'fantasy'}}>LOGIN</h1><br/>
                <div class="row-login">
                        ...
                        <div className="form-group">
                            ...
                            <Link to="/researcherforgotpassword" tabIndex={4} style={{float:'right',marginRight:'20px',fontSize:'14px', fontWeight:"bolder", fontFamily:'cursive'}}>Forgot password?</Link>
                            <br/>
                            <hr/>
        
                            <div className="form-group" style={{marginLeft:'30%'}}>
                                <button type="submit" onClick={this.researcherLoginHandler} className="form-btn-login" tabIndex={3}>Login</button>
